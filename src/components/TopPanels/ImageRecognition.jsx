...
const DEFAULT_MODEL_PATH = '/models/best.onnx'
...
export default function ImageRecognition({ onClose }) {
  ...
  const modelHint = useMemo(() => '默认模型路径：public/models/best.onnx（也可直接上传 onnx 文件）', [])
  ...
  const ensureBrowserSession = async () => {
    ...
    ort.env.wasm.wasmPaths = source === 'local' ? '/vendor/' : 'https://cdn.jsdelivr.net/npm/onnxruntime-web/dist/'
    ...
    try {
      ...
      setError(
        `${err.message || 'Inference failed'}。` +
          '若看到 best.onnx 404，请先放置 public/models/best.onnx；若 CDN 被拦截，请把 ort.min.js 放到 public/vendor/。'
      )
    } finally {
      setLoading(false)
    }
